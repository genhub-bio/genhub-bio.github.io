...
changes, including genome/chromosome sized sequences.  Gen got its start at
Ginkgo Bioworks, and in this post we'll go through a simplified version of an
example synthetic biology workflow that one might see at Ginkgo.  We envision
other uses for gen, but hopefully this example will show enough of its
capabilities to make things interesting.

To keep this example realistic but as simple as possible, we're going to make a
single deletion to an E. coli genome and then incorporate the resulting
...
does match the E. coli reference fasta.)
...
Next, we import the [E coli reference fasta][e-coli-reference] obtained from NCBI:
...
We can list the "graphs" (contigs) of the current sample to confirm U00096.3 got imported:
...
Now, we make our intentional edit, a partial deletion of the lacZ gene.  This
deletion is a well-known one and produces strains that can be used for further
experiments.
...
We confirm the edit by getting a diff in GFA format between the default
(original) sample and the child, and then inspecting the GFA:
...
Next, assuming we've done the edit in the lab and put it through sequencing and
have a VCF file capturing the variants, we apply that VCF:
...
We can export the result after sequencing as a fasta:
...
Internally, gen tracks the sequence changes as additions to a graph (along the
lines of a pangenome graph).  We can output the entire graph generated by our
updates in GFA format:
...
coordinates.  We have a reference GFF with lacZ.  We can propagate it and then
grep for "lacZ" in the result as a way to check the deletion happened (the
coordinate range should be smaller in the sequence of the descendant):
...
The resulting GFF file has lacZ annotations with start/end coordinates updated
...
repository to colloborate on.  We're working on a web frontend that will people
...
If gen looks interesting to you, please try it out.  [There are builds
available for Mac OS X and Linux][gen-nightly-builds].  Be aware that it is
still in active development, so it is quite possible you will see bugs.  But we
are committed to making this software better, so please let us know if you have
